...
def get_wikitext2(nsamples, seed, seqlen, model, hf_token, eval_mode=False):
    ...
    if eval_mode:
        testdata = pd.read_parquet('/mnt/data1/lishuaiting/llmsource/wikitext/wikitext-2-raw-v1/test-00000-of-00001.parquet')
        # testdata = datasets.load_dataset('wikitext', 'wikitext-2-raw-v1', split='test')
        testenc = tokenizer("\n\n".join(testdata['text']), return_tensors='pt')
        return testenc
    else:
        traindata = pd.read_parquet('/mnt/data1/lishuaiting/llmsource/wikitext/wikitext-2-raw-v1/train-00000-of-00001.parquet')
...
def get_c4_new(nsamples, seed, seqlen, model, hf_token=None, eval_mode=False):
    ...
    cache_path = "/home/lishuaiting/my_huggingface_cache" 

    if eval_mode:
        # valdata = datasets.load_dataset(
        # 'allenai/c4', data_files={'validation': 'en/c4-validation.00000-of-00008.json.gz'}, split='validation')
        valdata = datasets.load_dataset('json', data_files={'validation': "/mnt/data1/lishuaiting/llmsource/c4-validation.00000-of-00008.json.gz"}, split='validation', cache_dir=cache_path)
        ...
        traindata = datasets.load_dataset('json', data_files={'train': "/mnt/data1/lishuaiting/llmsource/c4-train.00000-of-01024.json.gz"}, split='train', cache_dir=cache_path)
        random.seed(seed)
        trainloader = []
        for _ in range(nsamples):
            while True:
